...
Throughout this document, transverse deflection $w(x)$ is taken as \textbf{positive downward}, consistent with the direction of gravity loading, and $q(x)$ is likewise positive downward. This is the standard structural engineering sign convention.
...
\begin{equation}
  ...
  \label{eq:moment}
\end{equation}
The negative sign arises from the downward-positive $w$ convention: a sagging beam has $w'' < 0$, so $M = -EI w'' > 0$ gives a positive moment for sagging, consistent with the engineering sign convention.
...
\begin{equation}
  ...
  \label{eq:load}
\end{equation}
where $q_{\text{roof}}$ and $q_{\text{floor}}$ are area loads [psf] and $T_w$ is the tributary width [ft], giving $q_0$ in [lb/ft].
Before numerical solution all quantities must be in a consistent unit system.
Converting to inch-based units: $q_0\,[\text{lb/in}] = q_0\,[\text{lb/ft}] / 12$, with $L$ in inches, $E$ in psi (lb/in$^2$), $I$ in in$^4$, and $w$ in inches.
...
\begin{enumerate}
  ...
  \item \textbf{Deflection:} $w_{\max} \leq L/240$ (IRC serviceability limit, with $w_{\max}$ and $L$ in the same units)
\end{enumerate}
