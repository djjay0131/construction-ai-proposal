...
The discretized governing equation at each interior point $i = 2, \ldots, N-2$ is:
...
\textbf{Simply-supported boundary conditions} modify the first and last rows of $\mathbf{K}$ to enforce:
\begin{align}
  w_0 &= 0 \quad \Rightarrow \quad \text{row } 0: \; [1, 0, \ldots] \\
  w_N &= 0 \quad \Rightarrow \quad \text{row } N: \; [\ldots, 0, 1] \\
  w_0'' = 0 &\Rightarrow \quad \frac{w_{-1} - 2w_0 + w_1}{h^2} = 0
              \;\;\text{with ghost point } w_{-1} = -w_1 \\
  w_N'' = 0 &\Rightarrow \quad \frac{w_{N-1} - 2w_N + w_{N+1}}{h^2} = 0
              \;\;\text{with ghost point } w_{N+1} = -w_{N-1}
\end{align}

The system~\eqref{eq:linear-system} is solved using a direct banded solver (LU factorization), exploiting the pentadiagonal structure for $\mathcal{O}(N)$ cost.
...
\begin{equation}
  ...
  \mathbf{1}\!\left(\sigma_{\max}^{(j)} > F_b \;\cup\; \tau_{\max}^{(j)} > F_v \;\cup\; w_{\max}^{(j)} > \frac{L}{240}\right)
  \label{eq:pfail}
\end{equation}
...
UQ method & Monte Carlo sampling ($N_{\text{MC}} = 1{,}000$--$10{,}000$) \\
