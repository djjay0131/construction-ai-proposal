...
\begin{tabular}{llll}
  ...
  \midrule
  Span              & $L$      & 96.0      & in \\
  Width             & $b$      & 3.5       & in (double-ply 1.75-in wide LVL) \\
  Depth             & $d$      & 11.25     & in \\
  ...
  Flexural rigidity & $\EI$    & $8.306\times10^8$ & lb$\cdot$in$^2$ \\
  \bottomrule
\end{tabular}
\end{center}
...
The closed-form reference is $\wmax = 5q_0L^4/(384\EI) = 0.05548$~in (nominal $E$).
...
\begin{equation}
  ...
  \quad
  \muE = 2{,}000{,}000\ \text{psi},
  \quad
  \sigE = 200{,}000\ \text{psi}
  \quad
...
A 10\,\% coefficient of variation for LVL elastic modulus is the upper bound
required by ASTM~D5457 (reliability-based reference resistance) and enforced
through the ASTM~D5456 acceptance-testing procedure for Microllam~2.0E.
The $\pm 1\sigma$ range spans $E \in [1.80, 2.20]\times10^6$~psi,
enclosing the Microllam 2.0E production grade per ESR-1387.
...
\begin{tabular}{ccc}
  ...
  \midrule
  $E = \muE - \sigE$ & 1,800,000 & $\beta = 0.0617680$  (larger, $\alpha$ = max SRQ) \\
  $E = \muE + \sigE$ & 2,200,000 & $\alpha = 0.0505374$  (smaller, minimum SRQ) \\
  \bottomrule
\end{tabular}
\end{center}

The range $\beta - \alpha = 0.0112305$~in represents the $\pm 1\sigma$
...
\caption{LHS sample statistics for $E$ and propagated $\wmax$ at each sample size.
  True distribution parameters: $\muE = 2{,}000{,}000$~psi, $\sigE = 200{,}000$~psi;
  nominal $\wmax = 0.05548$~in.}
\label{tab:lhs}
\small
\begin{tabular}{rccccccc}
  ...
  $w_{\min}$ [in] & $w_{\max}$ [in] \\
  \midrule
   10 & 1,999,908 & 224,109 & 0.056331 & 0.006665 & 0.045931 & 0.071563 \\
   25 & 2,003,447 & 200,673 & 0.056056 & 0.005637 & 0.045055 & 0.068331 \\
  100 & 1,999,557 & 196,755 & 0.056157 & 0.005677 & 0.045068 & 0.072783 \\
  \bottomrule
\end{tabular}
\end{table}
...
$\bar{w} \approx 5q_0L^4/(384 \muE I) \cdot (1 + \mathrm{CoV}^2) \approx 0.05548\times1.01 = 0.05603$~in,
and the theoretical standard deviation $s_w \approx \wmax^{\mathrm{nom}} \cdot \mathrm{CoV} = 0.005548$~in.
...
All five experimental points lie at or above $\beta = 0.0618$~in,
...
\begin{tabular}{rcccl}
  ...
  \midrule
   10 & 0.006497 & 0.005661 & 0.871 & under-predicts (unconservative) \\
   25 & 0.006013 & 0.005937 & 0.987 & under-predicts (unconservative) \\
  100 & 0.006072 & 0.005835 & 0.961 & under-predicts (unconservative) \\
  \bottomrule
\end{tabular}
\end{table}
...
experiments by approximately $5.8$--$5.9 \times 10^{-3}$~in on average.
...
places the distribution centroid closer to the simulation mean of $\approx 0.056$~in.
...
\begin{tabular}{rcccl}
  ...
  $n_{\mathrm{sim}}$ & AVM [in] & MAVM [in] & $|\MAVM/\AVM|$ & Interpretation \\
  \midrule
   10 & 0.004173 & 0.003561 & 0.853 & under-predicts (unconservative) \\
   25 & 0.003837 & 0.003837 & 1.000 & under-predicts (fully one-sided) \\
  100 & 0.003893 & 0.003735 & 0.961 & under-predicts (unconservative) \\
  \bottomrule
...
\begin{enumerate}
  \item \textbf{$n_{\mathrm{sim}} = 10$:}  Both AVM and MAVM are somewhat elevated
        relative to the converged values.  Dataset~1 AVM $= 6.50\times10^{-3}$~in
        vs.\ the $n=100$ value of $6.07\times10^{-3}$~in (7\,\% overestimate).
  ...
  \item \textbf{$n_{\mathrm{sim}} = 100$:}  The finest estimate.  AVM and MAVM change
        ...
        ($6.07$ vs.\ $6.01 \times10^{-3}$~in, $< 1\,\%$) reflects sampling variability
...
$2{,}000{,}000$~psi assumed in the model.  The synthetic data were constructed with
...
\begin{tabular}{lcc}
  ...
  \midrule
  Dataset 1, $n=10$  & 6.50 & 5.66 \\
  Dataset 1, $n=25$  & 6.01 & 5.94 \\
  Dataset 1, $n=100$ & 6.07 & 5.83 \\[3pt]
  Dataset 2, $n=10$  & 4.17 & 3.56 \\
  Dataset 2, $n=25$  & 3.84 & 3.84 \\
  Dataset 2, $n=100$ & 3.89 & 3.74 \\
  \bottomrule
\end{tabular}
\end{center}
