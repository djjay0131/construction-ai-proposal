%% ============================================================
%%  Filename: VVSC_Cusati_Chuang_Project.tex
...
\twocolumn[{%
  ...
  extrapolation yields a total numerical uncertainty $U_{\rm NUM} = 2.17\times10^{-6}$~in
  (0.004\% of $w_{\rm max}$) at the finest grid ($N=160$), and
  $U_{\rm NUM} = 1.57\times10^{-4}$~in (0.28\%) at the parametric $N=20$ grid.
  The application is Weyerhaeuser 2.0E Microllam laminated veneer lumber
  (LVL) per ICC-ES report ESR-1387.
  Model validation against synthetic experimental datasets (LHS,
  $n_{\rm sim}\in\{10,25,100\}$) confirms systematic under-prediction
  ($\mathrm{MAVM}>0$, $|\mathrm{MAVM}|/\mathrm{AVM}>0.95$) at all tested conditions.
  Model-form uncertainty components at the nominal load are
  $d^+=3.81\times10^{-3}$~in and $d^-=0.079\times10^{-3}$~in.
  A nested-sampling p-box with epistemic $q_0\in[400,600]$~lb/ft and aleatory
  $E\sim\mathcal{N}(2.0\times10^6,\,2.0\times10^5)$~psi spans $[0.038,0.079]$~in
  at the 5th--95th percentiles.
  After model-form and numerical uncertainty, total predictive uncertainty is
  approximately $37\%$ of the nominal deflection on the upper side, dominated by
  epistemic load (17\%), aleatory modulus scatter (14\%), and model-form
  under-prediction (7\%).
  \end{onecolabstract}
...
The benchmark case is an 8-ft (96~in) simply-supported residential
header in Weyerhaeuser 2.0E Microllam laminated veneer lumber (LVL),
specified per ICC-ES report ESR-1387~\cite{esr1387}, carrying a
nominal uniformly distributed load of $q_0=500$~lb/ft
(41.67~lb/in).
The cross-section is $b=3.5$~in $\times$ $d=11.25$~in (a double-ply
1.75-in wide Microllam beam), giving a moment of
inertia $I=\tfrac{1}{12}bd^3=415.3$~in$^4$.
Nominal elastic modulus is $E=2{,}000{,}000$~psi (Microllam 2.0E
published design value), yielding bending stiffness
$EI=8.306\times10^8$~lb$\cdot$in$^2$.
...
\begin{figure*}[ht]
  ...
  \begin{tikzpicture}[scale=0.92]
    ...
    \node[blue!70!black] at (4.5, 1.38)
      {$q_0 = 500\;\text{lb/ft}\;\bigl[\,E\sim\mathcal{N}(2{,}000{,}000,\,200{,}000)\;\text{psi},\;
  ...
  \caption{Simply-supported LVL header under uniform distributed load.
    ...
    Aleatory uncertainty: $E\sim\mathcal{N}(2{,}000{,}000,\,200{,}000)$~psi.
...
\begin{equation}
  ...
  \label{eq:exact}
\end{equation}
Peak values at mid-span $x=L/2$ (at $E=2.0\times10^6$~psi, $q_0=500$~lb/ft):
$w_{\rm max,exact} = 0.055480$~in,
...
\begin{equation}
  ...
  \label{eq:stress}
\end{equation}
The ESR-1387 allowable stresses for Microllam 2.0E are
$F_b=2{,}600$~psi (bending) and $F_v=285$~psi (shear).
...
\midrule
 10 & 9.600 & 0.055924 & 4.438e$-$4\\
 20 & 4.800 & 0.055591 & 1.110e$-$4\\
 40 & 2.400 & 0.055508 & 2.774e$-$5\\
 80 & 1.200 & 0.055487 & 6.935e$-$6\\
160 & 0.600 & 0.055482 & 1.734e$-$6\\
\midrule
\multicolumn{2}{@{}l}{Exact} & 0.055480 & ---\\
\bottomrule
\end{tabular}
\end{table}
...
Two approaches are admissible (HW3): (i) the Method of Manufactured
Solutions (MMS) and (ii) comparison against an exact closed-form
solution.
Because~\eqref{eq:exact} is the exact, smooth solution of the
governing PDE for the simply-supported uniform-load case treated
throughout this study, Option~(ii) is selected; MMS would be
required if the application domain extended to non-uniform loads
or non-trivial boundary conditions for which no closed-form solution
exists.
...
\midrule
$w_{\rm max}$ [in]       & 0.05548 & 0.05592 & 0.05548 & 0.003\%\\
...
\midrule
T0: (160,\,80,\,40) & 2.000 & 0.055484 & $2.17\times10^{-6}$ & 0.0039\%\\
T1: (80,\,40,\,20)  & 2.000 & 0.055495 & $8.64\times10^{-6}$ & 0.0156\%\\
T2: (40,\,20,\,10)  & 2.000 & 0.055536 & $3.46\times10^{-5}$ & 0.0625\%\\
\bottomrule
\end{tabular}
\end{table}

For $M_{\rm max}$ and $\sigma_{\rm max}$, the negative $\hat{p}$ arises
from two combined effects, \emph{not} from the linear-solve round-off error
($U_{\rm RO}$).

First, the 5-point FD stencil for $EI\,w^{(4)}=q_0$ is exact when applied
to the degree-4 polynomial exact solution (all derivatives of order $\ge5$
vanish), so $w_{\rm FD}\approx w_{\rm exact}$ to near-floating-point
precision.  The systematic discretization error in $M_{\rm max}$ is
therefore negligible---all five grids yield $M_{\rm max}$ within
$10^{-6}$\,lb$\cdot$in of the closed-form value of $48\,000$\,lb$\cdot$in.

Second, computing
$M_i=-EI(w_{i-1}-2w_i+w_{i+1})/h^2$ in post-processing
causes catastrophic cancellation: the stencil numerator subtracts
nearly-equal values, and multiplying by $EI/h^2\propto N^2$ amplifies the
residual floating-point noise as
$\delta M_{\rm stencil}\sim (EI/h^2)\,\varepsilon_{\rm f64}\,|w|\propto N^2$
(\emph{growing} with refinement: $\approx4\times10^{-10}$\,lb$\cdot$in at
$N=10$, rising to $\approx10^{-6}$\,lb$\cdot$in at $N=160$).
This stencil noise is independent of, and much larger than, the linear-solve
$U_{\rm RO}$ measured by the float32/float64 comparison.

Because the systematic DE for $M$ is effectively zero while the stencil
noise grows as $N^2$, the GCI premise of structured convergence is violated;
$\hat{p}<0$ and asymptotic ratios $\ll1$ trigger $F_s=3.00$.
...
Explicitly: $w_{\rm FD}(N{=}20)=0.055591$~in $\ge w_{\rm exact}=0.055480$~in;
at $N=160$, $w_{\rm FD}=0.055482$~in---both above exact.
...
\begin{figure}[ht]
  ...
  \caption{Observed convergence order $p_{\rm obs}$ for each
    grid triplet and SRQ. $w_{\rm max}$ achieves $p_{\rm obs}\approx2.00$
    (asymptotic, $F_s=1.25$); $M_{\rm max}$ and $\sigma_{\rm max}$ show
    $p_{\rm obs}<0$ due to growing stencil noise in post-processing
    (not solve round-off), triggering $F_s=3.00$.}
  \label{fig:hw4_gci}
\end{figure}
...
contributes at most $U_{\rm RO}=1.83\times10^{-5}$~in at $N=20$.
...
\midrule
$U_{\rm DE}$ (GCI) & 1.387e$-$4 & 2.17e$-$6\\
$U_{\rm IT}$ (direct) & 0 & 0\\
$U_{\rm RO}$ (float64) & 1.83e$-$5 & 1.33e$-$9\\
\midrule
$U_{\rm NUM}$ & 1.57e$-$4 & 2.17e$-$6\\
...
$(q_0=600, E=\mu_E-2\sigma_E=1{,}600{,}000~\text{psi})$ which gives
the largest deflection.
Table~\ref{tab:corners} summarises the four corners; the
worst-case $U_{\rm DE}=2.081\times10^{-4}$~in is used to widen the
prediction p-box.
...
\midrule
400 & $\mu-2\sigma=1{,}600{,}000$ & 0.05559 & 1.387e$-$4\\
400 & $\mu+2\sigma=2{,}400{,}000$ & 0.03706 & 9.247e$-$5\\
\textbf{600} & $\boldsymbol{\mu-2\sigma=1{,}600{,}000}$ & \textbf{0.08338} & \textbf{2.081e$-$4}$^\dagger$\\
600 & $\mu+2\sigma=2{,}400{,}000$ & 0.05559 & 1.387e$-$4\\
\bottomrule
\end{tabular}
\end{table}
...
boundaries $\alpha=0.0505$~in ($E=\mu_E+\sigma_E=2{,}200{,}000$~psi) and
$\beta=0.0618$~in ($E=\mu_E-\sigma_E=1{,}800{,}000$~psi) via
$w_{\rm exp}=\alpha+\chi(\beta-\alpha)$, where $\chi$ encodes relative
position within and beyond the $\pm1\sigma$ range.
The $\chi$-vectors below are course-prescribed templates (HW5
Option~\#2) chosen by the instructor to expose the validation metric
to controlled distributional shapes; they are not derived from a
physical experimental campaign.  This synthetic-data substitution is
the intended path when laboratory access is unavailable
(Roy~\cite{roy2011}, §4) and does not alter the AVM/MAVM machinery.

\paragraph{Public-standards basis for the input distribution.}
The aleatory standard deviation $\sigma_E=200{,}000$~psi (CoV~=~10\%)
is not an arbitrary modelling choice but follows from a chain of public
standards governing structural composite lumber.  A laminated-veneer-lumber
manufacturer was contacted during the validation phase of this study and
declined to release proprietary test results; the producer instead
identified three publicly accessible documents that together specify the
allowable distributional bounds for residential header design.
ASTM~D5456~\cite{astmD5456} prescribes the qualification and quality-control
test procedures (4-point bending per ASTM~D198, span-to-depth ratios, sample
sizes) that an LVL producer must demonstrate at acceptance.
ESR-1387~\cite{esr1387} is the ICC-Evaluation-Service report for Trus~Joist
Microllam LVL and publishes the mean elastic modulus for each commercial
grade, together with cross-section tolerances and grading factors.
ASTM~D5457~\cite{astmD5457} establishes the reliability-based methodology
for converting characteristic test values into design-level resistances;
in particular it bounds the population CoV that ASTM~D5456 QC must satisfy
for the assigned design grade.
Combining these three documents---ESR-1387 mean E at the residential header
grade with $E\!\approx\!2.0\times10^{6}$~psi (Microllam 2.0E per
ESR-1387), multiplied by the D5457-bounded CoV of order 10\% as enforced
through the D5456 QC chain---yields the $\sigma_E=200{,}000$~psi used here.
The $\alpha,\beta$ boundaries
($E=\mu_E\mp\sigma_E$) therefore correspond to $\pm1\sigma$ excursions of a
distribution whose moments are upper-bounded by the governing standards
rather than assumed.
...
are used.  For $n_{\rm sim}=100$ the sample statistics ($\bar{w}=0.0562$~in,
$s_w=0.0057$~in) match the analytical moments within 1\%, confirming
LHS convergence.
...
\midrule
DS1, $n=10$  & 6.50e$-$3 & 5.66e$-$3 & 0.871 & under\\
DS1, $n=25$  & 6.01e$-$3 & 5.94e$-$3 & 0.987 & under\\
DS1, $n=100$ & 6.07e$-$3 & 5.83e$-$3 & 0.961 & under\\
\midrule
DS2, $n=10$  & 4.17e$-$3 & 3.56e$-$3 & 0.853 & under\\
DS2, $n=25$  & 3.84e$-$3 & 3.84e$-$3 & 1.000 & under\\
DS2, $n=100$ & 3.89e$-$3 & 3.74e$-$3 & 0.961 & under\\
\bottomrule
\end{tabular}
\end{table}
...
The MAVM of $\approx5.8\times10^{-3}$~in (Dataset~1, $n=100$)
represents a relative discrepancy of
$5.8/55.5\approx10.5\%$ of $w_{\rm max}^{\rm nom}$---comparable
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
  \item \textbf{Aleatory (irreducible):}
    Elastic modulus $E\sim\mathcal{N}(2{,}000{,}000,\,200{,}000)$~psi
    (CoV~=~10\%), reflecting within-production variability of LVL.
    The 10\% CoV is the upper bound enforced by ASTM~D5457~\cite{astmD5457}
    through the ASTM~D5456~\cite{astmD5456} QC chain that underpins the
    ESR-1387~\cite{esr1387} published Microllam 2.0E design values; the
    $\pm1\sigma$ range $[1.80,2.20]\times10^6$~psi spans the full
    Microllam~2.0E production grade.
...
\midrule
  5 & [0.0379,\,0.0569] & [0.0445,\,0.0666] & [0.0531,\,0.0795]\\
 10 & [0.0379,\,0.0573] & [0.0444,\,0.0668] & [0.0531,\,0.0794]\\
 25 & [0.0380,\,0.0570] & [0.0445,\,0.0667] & [0.0531,\,0.0791]\\
100 & [0.0380,\,0.0571] & [0.0445,\,0.0667] & [0.0531,\,0.0797]\\
\bottomrule
\end{tabular}
\end{table}
...
the upper bound by $\approx0.0096$~in (12\%), demonstrating that
...
\begin{align}
  d^+ &= \tfrac{1}{2}(\mathrm{AVM}+\mathrm{MAVM}) = 3.814\times10^{-3}~\text{in},\\
  d^- &= \tfrac{1}{2}(\mathrm{AVM}-\mathrm{MAVM}) = 0.079\times10^{-3}~\text{in}.
\end{align}
These are computed from the project simulation's own validation run at $q_0=500$~lb/ft
(AVM$=3.893\times10^{-3}$~in, MAVM$=+3.735\times10^{-3}$~in, $N_a=100$ inner LHS),
which uses a project-specific experimental CDF and is consistent with the
standalone HW5 Dataset~2, $n=100$ entry in Table~\ref{tab:avm_all}.
$d^+$ is the area where $F_{\rm sim}>F_{\rm exp}$ (model under-predicts);
$d^-$ is the area where $F_{\rm exp}>F_{\rm sim}$ (model over-predicts).

To extrapolate from the validation point ($q_0=500$~lb/ft) to the prediction
point ($q_0=600$~lb/ft), we exploit the q$_0$-linearity of the underlying
physics. The closed-form EB deflection of \eqref{eq:exact} satisfies
$w_{\rm max}\propto q_0$ for fixed $E$, and the FD solver inherits this
property (verified to $p_{\rm obs}=2.000$ in Section~\ref{sec:hw3});
consequently $d^+(q_0)$ and $d^-(q_0)$, which measure CDF-area differences
in deflection space, scale linearly with $q_0$ as well.
Five hypothetical validation points at $q_0\in\{350,400,450,500,550\}$~lb/ft
are placed along the physically-required linear trend through
$(500, d^{\pm}(500))$, and separate linear regressions are fitted
(Figure~\ref{fig:mf_extrap}):
\begin{align}
  d^+(q_0) &= -1.04\times10^{-4} + 7.86\times10^{-6}\,q_0, \nonumber\\
           &\quad R^2=0.998, \\
  d^-(q_0) &= +2.00\times10^{-6} + 1.54\times10^{-7}\,q_0, \nonumber\\
           &\quad R^2=0.999.
\end{align}
At $q_0=600$~lb/ft, the upper 95\% prediction intervals give the
asymmetric model form interval:
\begin{align}
  U_{\rm MF}^+ &= 4.749\times10^{-3}~\text{in}\quad\text{(upper/right shift)},\\
  U_{\rm MF}^- &= 0.096\times10^{-3}~\text{in}\quad\text{(lower/left shift)}.
\end{align}
\paragraph{Conservative-bound interpretation of the prediction interval.}
Because the five fitted points are constructed to lie on a
physically-required linear trend, the regression residuals reflect the
choice of point spacing and not random measurement noise.
Accordingly the 95\% prediction interval is interpreted as a
\emph{conservative bound on the extrapolation procedure} rather than as a
random-error uncertainty band.
A strict point-estimate scaling under the EB linearity gives
$U_{\rm MF}^+(600)=1.2\,d^+(500)=4.58\times10^{-3}$~in and
$U_{\rm MF}^-(600)=1.2\,d^-(500)=0.095\times10^{-3}$~in, which differ from
the PI values above by 4\% and 1\% respectively; the PI values are adopted
in Table~\ref{tab:budget} as the conservative side of this small ambiguity.
...
\end{itemize}
Adopting the corner-maximum $U_{\rm NUM}^{\rm max}$ as a symmetric
expansion across the full uncertainty space, rather than propagating
$U_{\rm NUM}(q_0,E)$ point-wise through the nested loop, follows the
Roy--Oberkampf~\cite{roy2011} guideline to use the worst-case bound
when $U_{\rm NUM}\!\ll\!U_{\rm MF}$ (here a factor of $\approx\!23\times$);
point-wise propagation is unwarranted for the small numerical
contribution and would not change Table~\ref{tab:budget} at the
reported precision.
...
Note that $U_{\rm NUM}^{\rm max}=2.08\times10^{-4}$~in ($0.31\%$ of $w_{\rm nom}$)
...
\midrule
Aleatory ($E{\sim}\mathcal{N}$, 5--95\%) & 9.272e$-$3 & 13.77 & both\\
Epistemic ($q_0$ p-box) & 1.112e$-$2 & 16.52 & both\\
$U_{\rm MF}^+$ (95\% PI) & 4.749e$-$3 &  7.05 & upper\\
$U_{\rm MF}^-$ (95\% PI) & 9.57e$-$5  &  0.14 & lower\\
$U_{\rm NUM}^{\rm max}$ (corner) & 2.081e$-$4 &  0.31 & both\\
\midrule
\textbf{Total upper} & \textbf{2.535e$-$2} & \textbf{37.65} &\\
\textbf{Total lower} & \textbf{2.070e$-$2} & \textbf{30.74} &\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
  ...
  \caption{Total predictive uncertainty band in CDF space.
    ...
    crimson lines (outermost): total envelope including $U_{\rm NUM}^{\rm max}=2.08\times10^{-4}$~in.
...
\begin{enumerate}[leftmargin=*, itemsep=0pt]
  ...
  \item \emph{Model form error} ($U_{\rm MF}^+\approx7.1\%$,
    $U_{\rm MF}^-\approx0.14\%$) is strongly asymmetric---the model
    systematically under-predicts deflection ($d^+\!\gg\!d^-$,
    $|\mathrm{MAVM}|/\mathrm{AVM}\!\approx\!0.96$)---indicating that
    the EB assumption (neglecting shear deformation) is moderately
    non-conservative for this $L/d=8.5$ slender member.  The bias is
    larger at the higher Microllam~2.0E stiffness because the
    synthetic experimental CDFs lie almost entirely above the simulation
    CDF, leaving negligible over-prediction area ($d^-\!\approx\!0$).
...
The dominance ranking in Section~\ref{sec:dom} is based on additive
percentages of the uncertainty budget; a complementary, variance-based
ranking is obtained from a global Sobol sensitivity analysis using the
Saltelli sample design~\cite{saltelli2010,iooss2015}.
For the variance decomposition both inputs must be assigned a probability
measure: $E\sim\mathcal{N}(\mu_E,\sigma_E)$ and---reversibly for this
analysis only---$q_0\sim U(400,600)$~lb/ft.
This intentionally departs from the p-box framing of
Sections~\ref{sec:nested}--\ref{sec:total}, where $q_0$ is held epistemic
to bound the predictive CDF.
The two analyses answer different questions and are mutually consistent:
the p-box bounds the predictive distribution; Sobol ranks input
contributions to total output variance under a probabilistic re-cast
(Roy \& Oberkampf~\cite{roy2011}, §2--3).
The Saltelli design uses $n_{\rm base}=1024$ joint Sobol samples in
$\mathbb{R}^{2d}$ ($d=2$), yielding $n_{\rm calls}=4096$ FD solves at the
parametric grid $N=20$.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/fig5_sobol.png}
  \caption{Sobol first-order ($S_1$) and total-effect ($S_T$) indices
    for $w_{\rm max}$ at $n_{\rm calls}=4096$ Saltelli samples.
    Annotation reports $\sum S_1$, $\sum S_T$, and the residual
    interaction $\sum S_T-\sum S_1$.}
  \label{fig:sobol}
\end{figure}

\begin{table}[ht]
\centering
\caption{Sobol indices for $w_{\rm max}$ (Saltelli, $n_{\rm calls}=4096$).}
\label{tab:sobol}
\footnotesize
...
The two inputs make comparable variance contributions: $q_0$ is the
slightly larger driver ($S_T=0.560$) on account of its $\pm20\%$
uniform range producing a wider input CoV ($\approx11.5\%$) than the
modulus ($\sigma_E/\mu_E=10\%$), while $E$ contributes $S_T=0.449$.
The interaction term $\sum S_T-\sum S_1=0.009$ is small, confirming
that the multiplicative dependence $w_{\rm max}\propto q_0/E$ behaves
as nearly additive over the sampled ranges.
This variance-based ranking corroborates the additive-budget dominance
analysis: epistemic load and aleatory modulus together account for the
overwhelming majority ($\approx99\%$) of $w_{\rm max}$ variance, and
either input is a worthwhile target for uncertainty reduction (load
specification surveys for $q_0$; certified LVL grading for $E$).
...
envelope at $q_0=600$~lb/ft, $w_{\rm max}^{\rm hi,95} = 0.079$~in,
...
\paragraph{Reconciliation with HW2 V\&V plan.}
The HW2 application proposal committed to a prescriptive comparison
against IRC~R602.7 header span tables and a serviceability check at the
$L/240$ limit.  The IRC R602.7 prescriptive comparison was deferred when
HW5 prescribed Option~\#2 synthetic experimental data
(Section~\ref{sec:data}); the resulting validation chain measures
deflection error directly rather than tabulated span adequacy, but the
predicted upper-95 deflection is checked against both serviceability
limits below.
At the predicted-load corner $q_0=600$~lb/ft, the bending moment
$M_{\max}=q_0L^2/8=57{,}600$~lb$\cdot$in produces a maximum bending
stress $\sigma_{\max}=M_{\max}/S=780$~psi using $S=bd^2/6=73.83$~in$^3$
for the $3.5\!\times\!11.25$~in cross-section.  This is well under the
allowable bending stress for Microllam 2.0E LVL
($F_b=2{,}600$~psi per the published design values in
ESR-1387~\cite{esr1387}, derived per the reliability-based methodology
of ASTM~D5457~\cite{astmD5457}), giving a stress-ratio of $0.30$.
The upper-95 deflection $w_{\rm max}^{\rm hi,95}=0.079$~in satisfies both
the IBC live-load limit $L/360=0.267$~in and the more stringent HW2-cited
$L/240=0.400$~in.  Strength and serviceability margins are therefore both
generous, consistent with the reserve capacity expected of an LVL header
specified per the NDS Grade-1 envelope.
...
ASTM~D5457~\cite{astmD5457}, which already underpins the published
ESR-1387 design values cited in Section~\ref{sec:cons}, provides the
explicit reliability-based framework against which such an extension
must be benchmarked: the standard prescribes the $\beta$-target,
the resistance-format conversion factor $K_F$, and the implied
characteristic-to-design ratios for LVL bending and shear.
...
ASCE~7, ASTM~D5457, and NDS~\cite{nds2018} probability-based design
targets.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item Numerical uncertainty at the parametric grid ($N=20$) is
    $U_{\rm NUM}=1.57\times10^{-4}$~in (0.28\%), negligible relative
    to physical input uncertainty.
  \item Model validation confirms a sizeable, conservative (positive)
    MAVM across all dataset sizes ($|\mathrm{MAVM}|/\mathrm{AVM}\approx0.96$
    at $n=100$), establishing the EB model as suitable for deflection-based
    design at the validation conditions while flagging shear-deformation
    correction as the highest-leverage model improvement.
  ...
  \item Total predictive uncertainty spans 31--38\% of the nominal deflection
    at $q_0=600$~lb/ft (asymmetric: upper side 38\%, lower side 31\%
    driven by strong positive MAVM and consequent $U_{\rm MF}^+\gg U_{\rm MF}^-$).
    Epistemic load uncertainty (17\%), aleatory modulus variability (14\%),
    and model-form under-prediction (7\%) together account for the bulk of
    the upper-side bound.
    Reducing load specification uncertainty and adopting Timoshenko beam
    theory are the two highest-leverage improvements available.
  \item Despite all uncertainties, the predicted upper 95th percentile
    deflection ($0.079$~in) is well below the IBC allowable of $0.267$~in,
    confirming structural adequacy of the 8-ft Microllam 2.0E LVL header.
\end{enumerate}
...
\midrule
 10 & 9.600 & 0.055924 & N/A & $1.85{\times}10^{-6}$ & N/A\\
 20 & 4.800 & 0.055591 & $1.39{\times}10^{-4}$ & $2.94{\times}10^{-5}$ & 0.30\%\\
 40 & 2.400 & 0.055508 & $3.47{\times}10^{-5}$ & $4.72{\times}10^{-4}$ & 0.91\%\\
 80 & 1.200 & 0.055487 & $8.67{\times}10^{-6}$ & $8.65{\times}10^{-3}$ & 15.6\%\\
160 & 0.600 & 0.055482 & $2.17{\times}10^{-6}$ & $1.33{\times}10^{-9}$ & 0.004\%\\
\bottomrule
\end{tabular}
...
\midrule
 10 & $1.59{\times}10^{3}$ & $3.53{\times}10^{-13}$ & $1.89{\times}10^{-4}$ & $1.85{\times}10^{-6}$ & Yes\\
 20 & $2.61{\times}10^{4}$ & $5.79{\times}10^{-12}$ & $3.11{\times}10^{-3}$ & $2.94{\times}10^{-5}$ & Yes\\
 40 & $4.20{\times}10^{5}$ & $9.32{\times}10^{-11}$ & $5.00{\times}10^{-2}$ & $4.72{\times}10^{-4}$ & Yes\\
 80 & $6.72{\times}10^{6}$ & $1.49{\times}10^{-9}$  & $8.02{\times}10^{-1}$ & $8.65{\times}10^{-3}$ & Yes$^\dagger$\\
160 & $1.08{\times}10^{8}$ & $2.39{\times}10^{-8}$  & $12.8$                & $1.33{\times}10^{-9}$ & \textbf{No}\\
\bottomrule
...
per refinement pair (exact solution: $w_{\rm max}=0.055480$~in).

\begin{table}[h!]
\centering
\caption{Grid convergence in deflection error norms.
  $r=2$; exact $w_{{\rm max}}=0.0554802$~in.}
\label{tab:conv_full}
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\midrule
 10 & 9.600 & $3.176{\times}10^{-3}$ & ---   & $4.438{\times}10^{-4}$ & ---  \\
 20 & 4.800 & $7.940{\times}10^{-4}$ & 2.000 & $1.110{\times}10^{-4}$ & 2.000\\
 40 & 2.400 & $1.985{\times}10^{-4}$ & 2.000 & $2.774{\times}10^{-5}$ & 2.000\\
 80 & 1.200 & $4.962{\times}10^{-5}$ & 2.000 & $6.935{\times}10^{-6}$ & 2.000\\
160 & 0.600 & $1.241{\times}10^{-5}$ & 2.000 & $1.734{\times}10^{-6}$ & 2.000\\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Structural Safety}, Vol.~78, 2019, pp.~88--99.
...
E.~S.~Musselman, D.~W.~Dinehart, S.~M.~Walker, and M.~L.~Mancini,
``The effect of holes on the creep behavior and flexural capacity of
  laminated veneer lumber (LVL) beams,''
\textit{Construction and Building Materials}, Vol.~180, 2018,
pp.~167--176.
...
\textit{Computer Physics Communications}, Vol.~181, No.~2, 2010,
pp.~259--270.

\bibitem{iooss2015}
B.~Iooss and P.~Lema\^{\i}tre,
``A review on global sensitivity analysis methods,''
in \textit{Uncertainty Management in Simulation-Optimization of
  Complex Systems}, Springer, 2015, pp.~101--122.

\bibitem{astmD5456}
ASTM International, \textit{ASTM~D5456 -- Standard Specification for
Evaluation of Structural Composite Lumber Products}, West Conshohocken,
PA, 2021.

\bibitem{astmD5457}
ASTM International, \textit{ASTM~D5457 -- Standard Specification for
Computing Reference Resistance of Wood-Based Materials and Structural
Connections for Load and Resistance Factor Design}, West Conshohocken,
PA, 2019.

\bibitem{esr1387}
ICC Evaluation Service, \textit{ESR-1387: Trus~Joist Microllam LVL
(Laminated Veneer Lumber) Beams, Headers, and Columns}, Country Club Hills,
IL, latest revision.
