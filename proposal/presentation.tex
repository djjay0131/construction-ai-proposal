...
\node[agent, right=of infer] (structural) {Structural\\Hypothesis Agent};
\node[agent, right=of structural] (code) {Code \&\\Compliance Agent};
...
\draw[arrow] (qa) -- (infer);
\draw[arrow] (infer) -- (structural);
\draw[arrow] (structural) -- (code);
...
\foreach \i/\name/\color in {0/QA/vtmaroon, 60/Inference/vtorange, 120/Structural/vtmaroon, 180/Code/vtorange, 240/Procure/vtmaroon, 300/Instruct/vtorange} {
  \node[circle, draw=\color, thick, minimum size=1.5cm, fill=\color!10, font=\small] at (\i:3cm) {\name};
}
\node[circle, draw=vtgray, thick, minimum size=1.5cm, fill=vtgray!10, font=\small\bfseries] at (0,0) {KG};
\foreach \i in {0,60,120,180,240,300} {
  \draw[thick, vtgray, ->] (\i:1.1cm) -- (\i:2.2cm);
}
\end{tikzpicture}
...
% =============================================================================
% SECTION 3.5: STRUCTURAL HYPOTHESIS EVALUATION (VVUQ)
% =============================================================================
...
\begin{itemize}
  \item Each \textbf{hypothesis} $=$ a candidate (load-bearing-wall set, tributary partition)
  ...
  \item Top 2--3 ranked by \textbf{cost}, \textbf{robustness}, \textbf{design flexibility}
